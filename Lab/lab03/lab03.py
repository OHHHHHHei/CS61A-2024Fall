...
def double_eights(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    if n % 10 == 8: # whether the first eight exist
        if n // 10 % 10 == 8:
            return True
        
    if n // 10 == 0: # if it is a one digit number
        return False
    else:
        return double_eights(n // 10) # recursion
...
def make_onion(f, g):
    ...
    def can_reach(x, y, limit):
        if limit < 0: # base case 1
            return False
        elif x == y: # base case 2
            return True
        else: # recursion
