...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while now_place.is_hive == False and now_place != None:
            if now_place.bees != [] and self.lower_bound <= step <= self.upper_bound:
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        if other.is_container is True or self.ant_contained is not None:
            return False
        else:
            return True
        # END Problem 8a
        ...
        while(now_place != None):
            if now_place.ant:
                if isinstance(now_place.ant, ContainerAnt):
                    if now_place.ant.ant_contained:
